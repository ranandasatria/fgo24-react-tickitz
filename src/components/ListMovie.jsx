...
function ListMovie({ sortOption }) {
  ...
  useEffect(() => {
    const fetchMovies = async () => {
      try {
        ...
        const sortParam = sortOption.toLowerCase().replace('name (a-z)', 'name-asc').replace('name (z-a)', 'name-desc');
        const res = await http().get(
          `/movies/now-showing?page=${currentPage}&limit=${moviesPerPage}&search=${encodeURIComponent(searchQuery)}&genres=${genresParam}&sort=${sortParam}`
        );
        setMovies(res.data.results?.movies || []);
        setTotalRows(res.data.results?.total || 0);
      } catch (error) {
    ...
    fetchMovies();
  }, [currentPage, searchQuery, selectedGenres, sortOption]);
  ...
  const totalPages = Math.ceil(totalRows / moviesPerPage);
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`flex h-8 w-8 sm:h-10 sm:w-10 md:h-[3.375rem] md:w-[3.375rem] items-center justify-center rounded-full cursor-pointer ${
                    currentPage === page ? 'bg-primary-500' : 'border border-black-500'
                  }`}
                >
